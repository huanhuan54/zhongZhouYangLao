chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
    ...
    if (request.action == "trans_google") {
        ...
        fetch("https://www.easyscholar.cc/extension/listPublicationRank9", {
            ...
            if (response.ok) {
                ...
                let tempResponse = {};
                tempResponse["msg"] = "连接超时，若您当前处于国外，那么可能由于跨境访问延时过高，您可以更换网络，或稍后再试。问题若一直存在，可以联系客服排查" + response.status;
                tempResponse["code"] = 199;
                response = tempResponse;
            ...
            sendResponse(response);
                ...
                let tempResponse = {};
                tempResponse["msg"] = "您的网络连接至easyScholar服务器超时，请稍后再试。您可以按照以下步骤依次排查问题： " +
                    "1.浏览器扩展的权限，需要为默认设置（允许运行在所有网站上）" +
                    "2.使用手机（运营商流量）访问官网，若同样无法访问，则可能已停止服务，请您及时和我们反馈。若可以访问，则继续尝试如下步骤。 " +
                    "3.若您使用了VPN，可能由于节点超时，您可以切换其他VPN节点、关闭VPN使用谷歌学术国内镜像、将本网站添加进VPN白名单等。 " +
                    "4.若是校园网（或单位内部网络），请尝试设置其他公共DNS服务器，或寻找本校校园网备用DNS服务器地址。 " +
                    "5.若不符合上述两种情况，且在电脑上能访问官网，请卸载重装试试。"
                tempResponse["code"] = 199;
...
chrome.runtime.onInstalled.addListener(details => {
    if (details.reason === chrome.runtime.OnInstalledReason.INSTALL) {
        ...
        fetch(
            "https://www.easyscholar.cc/api/index/getConfig?name=defaultscihub", {
                method: "get",
                credentials: "include"
            }
        ).then(response => response.json()).then(function (response) {
            let data = "https://www.sci-hub.vg/"
